...
class SupervisedForecastTask(pl.LightningModule):
    def __init__(
        self,
        model: nn.Module,
        regressor="linear", # linear, logistic
        loss="mse", # mse, mse_with_regularizer, CrossEntropyLoss
        pre_len: int = 3,
        ...
        **kwargs
        ...
        self.mlplayers = nn.Sequential(
            nn.Linear(self.hparams.adjust_feat+self.hparams.pre_len, self.hparams.pre_len)
        ) # Added by TC
    ...
    def forward(self, x):
        ...
        x = torch.swapaxes(self.mlplayers(torch.swapaxes(x, 1, 2)), 1, 2)
        hidden = self.model(x)
        # (batch_size * num_nodes, hidden_dim)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        elif self.hparams.num_class==4:
            predictions[(0 < predictions) & (predictions <= 0.5)] = 0
            predictions[(0.5 < predictions) & (predictions <= 1.5)] = 1
            predictions[(1.5 < predictions) & (predictions <= 2.5)] = 2
            predictions[(2.5 < predictions)] = 3
        elif self.hparams.num_class==5:
            predictions[(0 < predictions) & (predictions <= 0.5)] = 0
            predictions[(0.5 < predictions) & (predictions <= 1.5)] = 1
            predictions[(1.5 < predictions) & (predictions <= 2.5)] = 2
            predictions[(2.5 < predictions) & (predictions <= 3.5)] = 3
            predictions[(3.5 < predictions)] = 4
        else:
            predictions[(0 < predictions) & (predictions <= 0.5)] = 0
            predictions[(1.5 < predictions)] = 1
